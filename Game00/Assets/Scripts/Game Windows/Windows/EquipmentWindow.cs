...
public class EquipmentWindow : SlotWindow {
    ...
    protected override void CreateWindow()
    {
        ...
        int columns = 2;
        int width = columns * slotSize + columns * slotPadding + slotSize*3;
        ...
        for (int y = 0; y < slotTotal / columns; y++)
        {
            for (int x = 0; x < columns; x++)
            {
                ...
                slotRect.localPosition = new Vector3(slotPadding + slotSize * (4 * x), -slotPadding * (y + 1) - slotSize * y - titleSize);
