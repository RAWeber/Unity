...
public class MeshGenerator : MonoBehaviour
{
    ...
    public void GenerateMesh(int[,] map, float squareSize, float wallHeight)
    {
        ...
        GenerateFloorMesh(map, squareSize, wallHeight);
    ...
    void CreateWallMesh(float wallHeight)
    {
        ...
        for (int i = 0; i < wallVertices.Count; i++)
        {
            ...
            {
                case 0:
                    uvs[i] = new Vector2(factor, 1);
                    march++;
                    break;
                case 1:
                    uvs[i] = new Vector2(factor + incr, 1);
                    march++;
                    break;
                case 2:
                    uvs[i] = new Vector2(factor, 0);
                    march++;
                    break;
                case 3:
                    uvs[i] = new Vector2(factor + incr, 0);
                    factor += incr;
                    march = 0;
                    break;
            }
    ...
    public void GenerateFloorMesh(int[,] map, float squareSize, float wallHeight)
    {
        ...
        //USES OVERLOADED SQUAREGRID CONSTRUCTOR
    ...
    public class SquareGrid
    {
        ...
        public SquareGrid(int[,] map, float squareSize, float _wallHeight)
        {
            int nodeCountX = map.GetLength(0);
            int nodeCountY = map.GetLength(1);
            float mapWidth = nodeCountX * squareSize;
            ...
            for (int x = 0; x < nodeCountX; x++)
            {
                for (int y = 0; y < nodeCountY; y++)
                {
                    Vector3 pos = new Vector3(-mapWidth / 2 + x * squareSize + squareSize / 2, _wallHeight, -mapHeight / 2 + y * squareSize + squareSize / 2); //SETS X TO BE _wallHeight (Note:By default wallHeight is negative)
                    controlNodes[x, y] = new ControlNode(pos, map[x, y] == 0, squareSize); //LOOKS FOR 0 INSTEAD OF 1
                }
            }
